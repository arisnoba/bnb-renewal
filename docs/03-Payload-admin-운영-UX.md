...
| `exam-school-logos` | `logoMedia` 업로드 관계 추가, 불필요한 legacy/admin 필드 제거, 슬러그 영문 안내/검증, 다운로드 리포트 기반 media 연결 완료 |

## 상담/오디션 폼 기준

Figma `BNB-Renewal-sRgb--26.04.01~`의 `문의하기 폼` 섹션을 기준으로 상담/지원 폼은 프론트 입력 경험을 먼저 확정한 뒤 Payload 컬렉션을 설계한다.

- 대상 폼은 `문의하기 폼`과 `오디션 지원하기 폼` 두 개다.
- 공식 Payload form builder는 간단한 제출 저장에는 사용할 수 있지만, 이 폼의 최종 운영 모델로 바로 쓰지 않는다.
- 이유는 문의 유형별 조건부 섹션, 파일 첨부, 오디션 사진/URL, 개인정보 동의, CAPTCHA, 운영 상태 관리가 필요하기 때문이다.
- 먼저 프론트에서 필드 노출, 필수값, 모바일 흐름, 첨부 UX, 개인정보 동의 문구를 확정한다.
- 프론트 구현 중에도 내부 필드 key는 임시 계약으로 고정해서 후속 DB 연결 때 이름을 크게 바꾸지 않는다.
- DB 설계 단계에서는 `inquiries`와 `audition-applications`처럼 운영자가 바로 볼 수 있는 전용 컬렉션을 만든다.
- 제출 데이터는 `submissionData: [{ field, value }]` 형태로만 쌓는 범용 테이블보다, 관리자 list/table에서 주요 컬럼과 상태를 볼 수 있는 구조를 우선한다.
- 제출 알림 메일이 필요하면 운영 배포 전 Payload email adapter를 별도 설정한다.

초기 필드 그룹:

| 폼 | 주요 그룹 | 후속 컬렉션 후보 |
| --- | --- | --- |
| 문의하기 | 문의 유형, 예약일, 신청자 정보, 제휴 신청, 연기 경험 정보, 개인정보 동의, 유입경로 | `inquiries` |
| 오디션 지원하기 | 신청자 정보, 오디션 정보, 사진 첨부, 유튜브/SNS URL, 개인정보 동의, 유입경로 | `audition-applications` |
