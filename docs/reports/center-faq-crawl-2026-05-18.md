...
| `sourceCenter` | 원문을 가져온 대표 센터. 공통 질문이라도 답변이 다르면 센터별 레코드 분리 검토 |
| `sourceUrl` | 레거시 원문 URL |
| `sourceOrder` | 레거시 페이지 내 노출 순서 |
| `status` | 초안 적재 시 `draft`, 검수 후 `published` 권장 |

### 공통 질문 적재 기준

- 질문과 답변이 모두 같은 항목은 `centers`에 여러 센터를 넣은 단일 FAQ로 적재할 수 있다.
- 질문은 같지만 답변, 수치, 링크, 대상 연령이 다른 항목은 같은 `question`을 쓰더라도 센터별 레코드로 분리하는 편이 안전하다.
- 나중에 하나의 질문 아래 센터별 답변 variant를 지원할 계획이 없다면, Payload 필터링을 위해 `centers`가 1개인 FAQ 레코드를 여러 개 만드는 구조를 권장한다.
...
질문 문구가 같거나 거의 같은 항목만 묶었다. 답변은 센터별 운영 정책과 수치가 달라질 수 있으므로, 아래 상세 항목에서 센터별 답변을 다시 검수해야 한다.

| 공통 질문 | centers | 답변 상태 | 권장 적재 방식 |
|---|---|---|---|
| 연기를 처음 배우는 사람들도 많이 있나요? | `art`, `exam`, `highteen`, `kids` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |
| 학원에 입학하기 전에 테스트를 받나요? | `art`, `exam`, `highteen`, `kids` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |
| 처음 입학해서 드라마, 영화에 출연하려면 평균 얼마나 걸리나요? | `art`, `highteen`, `kids` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |
| 청강이 가능한가요? | `art`, `exam`, `highteen`, `kids` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |
| 수강료는 어떻게 되나요? | `art`, `exam`, `highteen`, `kids` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |
| 기본 수업료 외에 학원 수업에 드는 추가 비용은 없나요? | `art`, `kids` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |
| 수강료 할인을 받을 수 있는 방법이 있나요? | `art`, `exam`, `kids` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |
| 수업 이수 과정 중에 다른 반, 다른 요일이나 시간대로 이동 가능 한가요? | `art`, `highteen`, `kids` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |
| 저는 연기 전공자입니다. 전공자들이 많이 다니나요? | `art`, `highteen` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |
| 엔터테인먼트 위탁배우들이 얼마나 되나요? | `art`, `highteen`, `kids` | 답변 동일 | `centers` 복수값 단일 레코드 가능 |
| 프로필은 꼭 찍어야 하나요? | `art`, `highteen`, `kids` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |
| 몇세부터 수강이 가능하나요? | `highteen`, `kids` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |
| 연기를 처음시작하는 경우 활동하는 학생들과 같이 교육을 하게 되나요? | `highteen`, `kids` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |
| 어떤 선생님이 교육을 진행하나요? | `highteen`, `kids` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |
| 반배정 및 승급 심사 기준이 어떻게 되나요? | `highteen`, `kids` | 답변 상이 또는 센터별 수치 포함 | 센터별 레코드 분리 권장 |

## 검수 메모

- 하이틴센터 sourceOrder 11은 질문이 `저는 연기 전공자입니다. 전공자들이 많이 다니나요?`이지만 답변은 학원 내부/방송국 오디션 운영 설명이다. DB 적재 전 원문 유지 또는 질문 교체 여부를 검수해야 한다.
...
### FAQ-01. 연기를 처음 배우는 사람들도 많이 있나요?

- centers: `art`, `exam`, `highteen`, `kids`
- 공통 여부: 공통 질문
- 권장 적재: 센터별 레코드 분리 권장
- 적재 메모: 질문은 공통으로 묶을 수 있으나 답변이 다르므로, 센터 필터 결과가 정확해야 하면 센터별 답변을 별도 레코드로 둔다.
...
- sourceOrder: 1
- question: 연기를 처음 배우는 사람들도 많이 있나요?
...
### FAQ-03. 처음 입학해서 드라마, 영화에 출연하려면 평균 얼마나 걸리나요?
...
- sourceUrl: https://baewoo.co.kr/web/bbs/content.php?co_id=faq
- sourceOrder: 5
- question: 처음 입학해서 드라마, 영화에 출연하려면 평균 얼마나 걸리나요?
...
### FAQ-06. 기본 수업료 외에 학원 수업에 드는 추가 비용은 없나요?
...
- sourceUrl: https://baewoo.co.kr/web/bbs/content.php?co_id=faq
- sourceOrder: 8
- question: 기본 수업료 외에 학원 수업에 드는 추가 비용은 없나요?
...
### FAQ-07. 수강료 할인을 받을 수 있는 방법이 있나요?
...
- sourceUrl: https://baewoo.co.kr/web/bbs/content.php?co_id=faq
- sourceOrder: 9
- question: 수강료 할인을 받을 수 있는 방법이 있나요?
...
- sourceUrl: https://baewoo.kr/web/bbs/content.php?co_id=faq
- sourceOrder: 4
- question: 수강료할인을 받을 수 있는 방법이 있나요?
...
### FAQ-08. 수업 이수 과정 중에 다른 반, 다른 요일이나 시간대로 이동 가능 한가요?
...
- sourceUrl: https://baewoo.co.kr/web/bbs/content.php?co_id=faq
- sourceOrder: 10
- question: 수업 이수 과정 중에 다른 반, 다른 요일이나 시간대로 이동 가능 한가요?
...
### FAQ-09. 저는 연기 전공자입니다. 전공자들이 많이 다니나요?
...
- sourceUrl: https://baewoo.co.kr/web/bbs/content.php?co_id=faq
- sourceOrder: 13
- question: 저는 연기 전공자입니다. 전공자들이 많이 다니나요?
...
- sourceUrl: https://baewoo.co.kr/web/bbs/content.php?co_id=faq
- sourceOrder: 14
- question: 엔터테인먼트 위탁배우들이 얼마나 되나요?
...
- sourceUrl: https://baewoo.me/web/bbs/content.php?co_id=faq
- sourceOrder: 4
- question: 연기를 처음시작하는 경우 활동하는 학생들과 같이 교육을 하게 되나요?
...
- sourceUrl: https://baewoo.net/web/bbs/content.php?co_id=faq
- sourceOrder: 4
- question: 연기를 처음시작하는 경우 활동하는 아이들과 같이 교육을 하게되나요?
...
### FAQ-15. 반배정 및 승급 심사 기준이 어떻게 되나요?
...
- sourceUrl: https://baewoo.me/web/bbs/content.php?co_id=faq
- sourceOrder: 13
- question: 반배정 및 승급 심사 기준이 어떻게 되나요?
...
### FAQ-17. 연기를 배워 본 적이 없는데 연기 전공자들하고 같이 수강하나요?
...
- sourceUrl: https://baewoo.co.kr/web/bbs/content.php?co_id=faq
- sourceOrder: 4
- question: 연기를 배워 본 적이 없는데 연기 전공자들하고 같이 수강하나요?
...
### FAQ-18. 연기 전공자는 아니지만 타 학원에서 수강을 한 적이 있습니다. Class 신청 기준이 어떻게 되나요?
...
- sourceUrl: https://baewoo.co.kr/web/bbs/content.php?co_id=faq
- sourceOrder: 11
- question: 연기 전공자는 아니지만 타 학원에서 수강을 한 적이 있습니다. Class 신청 기준이 어떻게 되나요?
...
### FAQ-22. 연습실 개수 및 사용시간대는 어떻게 되나요?
...
- sourceUrl: https://baewoo.kr/web/bbs/content.php?co_id=faq
- sourceOrder: 3
- question: 연습실 개수 및 사용시간대는 어떻게 되나요?
...
### FAQ-26. 연극영화과 세부전공 중 연기전공/뮤지컬전공 차이가 있나요?
...
- sourceUrl: https://baewoo.kr/web/bbs/content.php?co_id=faq
- sourceOrder: 10
- question: 연극영화과 세부전공 중 연기전공/뮤지컬전공 차이가 있나요?
...
- sourceUrl: https://baewoo.net/web/bbs/content.php?co_id=faq
- sourceOrder: 13
- question: 학원 내에서 자체적으로 오디션을 진행하기도 하나요?
...
- sourceUrl: https://baewoo.net/web/bbs/content.php?co_id=faq
- sourceOrder: 15
- question: 학원에 입학해야만 캐스팅이 진행되는건가요?
