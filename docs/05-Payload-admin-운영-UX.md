...
Figma `BNB-Renewal-sRgb--26.04.01~`의 `문의하기 폼` 섹션을 기준으로 상담 폼은 프론트 입력 경험을 먼저 확정한 뒤 Payload 컬렉션을 설계한다.

- 대상 폼은 `문의하기 폼` 하나다.
- 공식 Payload form builder는 간단한 제출 저장에는 사용할 수 있지만, 이 폼의 최종 운영 모델로 바로 쓰지 않는다.
- 이유는 문의 유형별 조건부 섹션, 제휴 파일 첨부, 개인정보 동의, CAPTCHA, 운영 상태 관리가 필요하기 때문이다.
- 먼저 프론트에서 필드 노출, 필수값, 모바일 흐름, 첨부 UX, 개인정보 동의 문구를 확정한다.
- 프론트 구현 중에도 내부 필드 key는 임시 계약으로 고정해서 후속 DB 연결 때 이름을 크게 바꾸지 않는다.
- DB 설계 단계에서는 `inquiries`처럼 운영자가 바로 볼 수 있는 전용 컬렉션을 만든다.
