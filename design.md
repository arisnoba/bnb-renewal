...
| 02 | 교육 | 등급제 교육관리시스템, 엔터테인먼트 위탁교육, 교육진 현황, 커리큘럼 | 교육 |
| 03 | 캐스팅 | BNB 출연장면, 캐스팅 출연현황, 캐스팅 센터, 배우 케어 시스템 | 캐스팅 |
...
| 02 | 교육 | 입시 매니지먼트, 특별한 시스템, 엔터테인먼트 위탁교육, 교육진 현황, 커리큘럼 | 입시센터 전용 |
| 03 | 합격현황 | 없음 · 대학교 합격현황으로 직접 이동 | 입시센터 전용 |
| 04 | 합격자소개 | 합격 후기, 합격 영상 | 입시센터 전용 |
| 05 | 지원센터 | NEWS&NOTICE, 입학안내, 학원100%이용법, 스타카드 멤버쉽서비스, 자주하는 질문 | 공통 |

### 하이틴센터 GNB 구조

하이틴센터(`highteen`)는 아트센터와 같은 1depth를 유지하되, 공개 GNB 2depth는 아래 구조를 기준으로 한다. 후보 메뉴였던 `대표인사말`, `커리큘럼`, `BNB 캐스팅`, `IMGround 캐스팅`, `BX모델에이전시`, `다이렉트 캐스팅`, `BNB 캐스팅 섭외뉴스`, `매니지먼트 시스템`, `프로필 촬영ㆍ제작`, `오디션 지원하기`는 하이틴 GNB 2depth에 노출하지 않는다.

| 순번 | 섹션 | 서브메뉴 | 비고 |
|------|------|---------|------|
| 01 | 배우앤배움 | 회사 소개, 센터 소개, 시설 안내, 오시는 길 | 하이틴센터 |
| 02 | 교육 | 등급제 교육관리시스템, 엔터테인먼트 위탁교육, 교육진 현황, 하이틴센터 특강 | 하이틴센터 |
| 03 | 캐스팅 | BNB 출연장면, 캐스팅 출연현황, 캐스팅 센터, 배우 케어 시스템 | 하이틴센터 |
| 04 | 아티스트 | BNB 루키 | 하이틴센터 |
| 05 | 지원센터 | NEWS&NOTICE, 입학안내, 학원100%이용법, 스타카드 멤버쉽서비스, 자주하는 질문 | 공통 |
...
| 02 | 교육 | 등급제 교육관리시스템, 엔터테인먼트 위탁교육, 커리큘럼, 교육진 현황 | 키즈센터 |
| 03 | 캐스팅 | BNB 출연장면, 캐스팅 출연현황, 캐스팅 센터, 배우 케어 시스템 | 키즈센터 |
| 04 | 아티스트 | BNB 루키 | 키즈센터 |
| 05 | 지원센터 | NEWS&NOTICE, 입학안내, 학원100%이용법, 스타카드 멤버쉽서비스, 자주하는 질문 | 공통 |

> **운영 비노출**: `/{center}/schedule`의 페이지와 관리자 기능은 유지하되, 촬영ㆍ오디션 스케줄은 공개 GNB, 메인 배너 바로가기, XML 사이트맵에 노출하지 않는다.
>
> **하이틴 커리큘럼 비노출**: `/highteen/curriculum` 페이지와 관리자 기능은 유지하되, 하이틴센터 커리큘럼은 공개 GNB, 센터 메인의 `BNB 강의검색`, XML 사이트맵에 노출하지 않는다.
>
> **하이틴·키즈 출신 아티스트 비노출**: `/highteen/artist-press`, `/kids/artist-press` 페이지와 관리자 기능은 유지하되, 해당 센터의 출신 아티스트는 공개 GNB와 XML 사이트맵에 노출하지 않는다. 두 센터의 메인 아티스트 섹션은 센터별 BNB 루키로 구성한다.
...
| 배우 케어 시스템 (Desktop) | 04캐스팅시스템_1 | [바로가기](https://www.figma.com/design/uJrO9GmJsQkqnKO6XLNoW6/BNB-Renewal-sRgb--26.04.01~-?node-id=457:10765) | ✅ `/art/casting-system`, `/highteen/casting-system`, `/kids/casting-system`, `/avenue/casting-system` |
...
| 구현됨 | 미구현 |
|--------|--------|
| `/audition` — 오디션 지원<br />`/{center}/screen-appearances` — BNB 출연장면<br />`/art/casting-system`, `/highteen/casting-system`, `/kids/casting-system`, `/avenue/casting-system` — 배우 케어 시스템<br />`/{center}/profile-production` — 프로필 제작 절차 안내 | 기업소개 전체 |
| `/news` — 뉴스 목록 | 교육 전체 |
