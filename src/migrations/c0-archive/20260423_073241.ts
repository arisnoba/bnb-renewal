...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TABLE "teacher_filmographies" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"source_db" varchar NOT NULL,
  	"source_table" varchar NOT NULL,
  	"source_id" numeric NOT NULL,
  	"slug" varchar NOT NULL,
  	"teacher_source_id" numeric NOT NULL,
  	"resolved_teacher_id" numeric,
  	"resolved_teacher_slug" varchar,
  	"resolved_teacher_name" varchar,
  	"title" varchar,
  	"poster_path" varchar,
  	"description" varchar,
  	"display_order" numeric DEFAULT 0,
  ...
  CREATE TABLE "payload_locked_documents_rels" (
  	...
  	"users_id" integer,
  	"teachers_id" integer,
  	"teacher_filmographies_id" integer,
  ...
  ALTER TABLE "payload_locked_documents_rels" ADD CONSTRAINT "payload_locked_documents_rels_teacher_filmographies_fk" FOREIGN KEY ("teacher_filmographies_id") REFERENCES "public"."teacher_filmographies"("id") ON DELETE cascade ON UPDATE no action;
  ...
  CREATE UNIQUE INDEX "teacher_filmographies_slug_idx" ON "teacher_filmographies" USING btree ("slug");
  CREATE INDEX "teacher_filmographies_updated_at_idx" ON "teacher_filmographies" USING btree ("updated_at");
  CREATE INDEX "teacher_filmographies_created_at_idx" ON "teacher_filmographies" USING btree ("created_at");
  ...
  CREATE INDEX "payload_locked_documents_rels_teacher_filmographies_id_idx" ON "payload_locked_documents_rels" USING btree ("teacher_filmographies_id");
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "teacher_filmographies" CASCADE;
