...
test('Word 문서의 확장자, MIME, 시그니처가 일치하면 상담 첨부로 허용한다', () => {
  const files = [
    {
...
test('확장자와 MIME이 다르면 거부한다', () => {
  ...
  assert.equal(
    uploadValidationMessage(result),
    '파일 확장자와 실제 파일 형식이 다릅니다. 파일 이름만 바꾸지 말고, 원본 파일을 열어 올바른 형식으로 다시 저장한 뒤 업로드해 주세요.',
  )
})
...
test('확장자와 MIME이 맞아도 시그니처가 다르면 거부한다', () => {
  ...
  assert.equal(
    uploadValidationMessage(result),
    '업로드한 파일을 읽을 수 없거나 확장자와 실제 파일 형식이 다릅니다. 파일 이름만 바꾸지 말고, 원본 파일을 열어 다시 저장한 뒤 업로드해 주세요. 이미지라면 JPEG, PNG 또는 WebP 형식으로 저장해 주세요.',
  )
})
