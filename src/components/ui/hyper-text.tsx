...
interface HyperTextProps extends Omit<MotionProps, "children"> {
  ...
  className?: string
  /** Optional className for each animated character */
  characterClassName?: string
  /** Disable the component's default text styles */
  unstyled?: boolean
...
export function HyperText({
  ...
  return (
    <MotionComponent
      ref={elementRef}
      className={cn(!unstyled && "overflow-hidden py-2 text-4xl font-bold", className)}
    ...
    >
      <AnimatePresence>
        {displayText.map((letter, index) => (
          <motion.span
            key={index}
            className={cn("font-mono", letter === " " ? "w-3" : "", characterClassName)}
          >
            {letter.toUpperCase()}
          </motion.span>
        ))}
