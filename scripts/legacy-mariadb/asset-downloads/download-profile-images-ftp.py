...
def main() -> None:
    ...
    entries = read_profile_entries(project_root)
    ...
    output = {
        ...
        "entries": entries,
        "generatedAt": now_iso(),
        "outputRoot": "public/legacy/profiles",
        "results": results,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="tmp/c0/profile-image-download-report.json")
...
def read_profile_entries(project_root: Path) -> list[dict[str, Any]]:
    sql = r"""
SELECT
  p.id,
  p.source_db,
  ...
  f.bf_no,
  REPLACE(REPLACE(REPLACE(COALESCE(f.bf_source, ''), CHAR(9), ' '), CHAR(10), ' '), CHAR(13), ' ') AS bf_source,
...
""".strip()
    ...
    for line in output.splitlines():
        ...
        local_path = f"public/legacy/profiles/{source_db}/{bo_table}/{source_id}/{bf_file}"
