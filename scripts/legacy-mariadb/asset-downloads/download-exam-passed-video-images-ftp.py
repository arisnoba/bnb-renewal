...
def main() -> None:
    ...
    entries = read_exam_passed_video_entries(project_root)
    ...
    output = {
        ...
        "entries": entries,
        "generatedAt": now_iso(),
        "outputRoot": "public/legacy/exam-passed-videos",
        "results": results,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="tmp/legacy-assets/exam-passed-video-image-download-report.json")
...
def read_exam_passed_video_entries(project_root: Path) -> list[dict[str, Any]]:
    sql = r"""
SELECT
  v.id,
  v.source_db,
  ...
  v.title,
  v.youtube_code,
  REPLACE(v.source_table, 'g5_write_', '') AS bo_table,
  f.bf_no,
  f.bf_source,
  f.bf_file,
  f.bf_filesize
FROM bnb_legacy_work.exam_passed_videos v
JOIN (
  ...
  ON f.source_db = v.source_db
  AND f.bo_table = REPLACE(v.source_table, 'g5_write_', '')
  AND f.wr_id = v.source_id
WHERE NULLIF(TRIM(f.bf_file), '') IS NOT NULL
  AND f.bf_no = 0
ORDER BY v.id, f.bf_no;
""".strip()
    ...
    for line in output.splitlines():
        ...
        local_path = f"public/legacy/exam-passed-videos/{source_db}/{bo_table}/{source_id}/{bf_file}"
