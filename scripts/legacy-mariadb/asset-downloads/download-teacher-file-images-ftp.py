...
class FTPPool:
    ...
    def download(self, entry: dict[str, Any], project_root: Path, dry_run: bool) -> dict[str, Any]:
        ...
        for candidate_dir in TEACHER_FILE_CANDIDATE_DIRS:
...
def main() -> None:
    ...
    entries = read_teacher_file_entries(project_root)
    ...
    output = {
        ...
        "entries": entries,
        "generatedAt": now_iso(),
        "outputRoot": "public/legacy/teacher-files",
        "results": results,
...
def read_teacher_file_entries(project_root: Path) -> list[dict[str, Any]]:
    sql = r"""
SELECT
  id,
  source_db,
  ...
  REPLACE(REPLACE(REPLACE(COALESCE(resolved_teacher_name, ''), CHAR(9), ' '), CHAR(10), ' '), CHAR(13), ' ') AS resolved_teacher_name,
  REPLACE(REPLACE(REPLACE(COALESCE(title, ''), CHAR(9), ' '), CHAR(10), ' '), CHAR(13), ' ') AS title,
  REPLACE(REPLACE(REPLACE(COALESCE(file_path, ''), CHAR(9), ' '), CHAR(10), ' '), CHAR(13), ' ') AS file_path,
  display_order
FROM bnb_legacy_work.teacher_files
WHERE resolved_teacher_id IS NOT NULL
  AND NULLIF(TRIM(file_path), '') IS NOT NULL
ORDER BY resolved_teacher_id, display_order, id;
""".strip()
        ...
        entries.append(
            {
                ...
                "displayOrder": parse_int(display_order),
                "filePath": file_path,
                "localPath": f"public/legacy/teacher-files/{source_db}/{source_table}/{file_path}",
                "remotePath": f"web/data/teacher/{file_path}",
                "resolvedTeacherId": parse_int(resolved_teacher_id),
                "resolvedTeacherName": resolved_teacher_name,
                "resolvedTeacherSlug": resolved_teacher_slug,
                "sourceDb": source_db,
                "sourceId": parse_int(source_id),
                "sourceTable": source_table,
                "sourceUrl": source_url(source_db, file_path),
                "title": title,
