...
async function main() {
  ...
  const rows = await parseInsertFile(
    resolveProjectPath('data', 'baewoo-curated', 'c0', 'g5_write_movie.sql'),
  )
  ...
  await upsertMovies(payload, limitedRecords)
...
function mapMovieRow(row: LegacyRow): MovieRecord {
  ...
  return {
    ...
    bodyHtml,
    castDescription: toNonEmptyString(row.wr_2),
    castLabel: toNonEmptyString(row.wr_1),
    ...
    sourceId,
    sourceTable: 'g5_write_movie',
    title: toNonEmptyString(row.wr_subject) ?? `movie-${sourceId}`,
...
async function upsertMovies(payload: Payload, records: MovieRecord[]) {
...
function printDryRun(records: MovieRecord[], options: SeedOptions) {
  const castLabelCounts = records.reduce<Record<string, number>>((accumulator, record) => {
    const key = record.castLabel ?? '(empty)'
