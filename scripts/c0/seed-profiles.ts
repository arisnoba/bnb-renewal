...
async function main() {
  ...
  const rows = await parseInsertFile(
    resolveProjectPath('data', 'baewoo-curated', 'c0', 'g5_write_new_profile.sql'),
  )
  ...
  await upsertProfiles(payload, limitedRecords)
...
function mapProfileRow(row: LegacyRow): ProfileRecord {
  ...
  return {
    ...
    name: toNonEmptyString(row.wr_subject) ?? `profile-${sourceId}`,
...
function printDryRun(records: ProfileRecord[], options: SeedOptions) {
