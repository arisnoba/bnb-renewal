...
type AppearanceExtraRecord = {
  ...
  slug: string
  sourceId: number
  sourceTable: 'g5_write_new_appear2'
  title: string
  viewCount: number
  writer?: string
}
...
async function main() {
  ...
  await upsertAppearancesExtra(payload, limitedRecords)
...
function mapAppearanceExtraRow(row: LegacyRow): AppearanceExtraRecord {
  ...
  return {
    ...
    slug: `appearance-extra-${sourceId}`,
    sourceId,
    sourceTable: 'g5_write_new_appear2',
    title: toNonEmptyString(row.wr_subject) ?? `appearance-extra-${sourceId}`,
...
async function upsertAppearancesExtra(payload: Payload, records: AppearanceExtraRecord[]) {
  const existing = await payload.find({
    collection: 'appearances-extra',
    depth: 0,
    limit: 500,
    pagination: false,
  })
...
function printDryRun(records: AppearanceExtraRecord[], options: SeedOptions) {
